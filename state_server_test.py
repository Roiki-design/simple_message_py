...
def feebackpublish(prot, client):
    print("sending feedback")
    prot.sendMsg(StandardMsgTypes.JOINT_FEEDBACK)
...
def main():
    ...
    # create endpoint
    trajData= TCP4ServerEndpoint(reactor=reactor, port=11000, backlog=50)
    feedback= TCP4ServerEndpoint(reactor=reactor, port=11002)
    ...
    d = feedback.listen(protocolFactory=factory)
    f = trajData.listen(protocolFactory=factory)
    ...
    f.addCallback(onStateEpConnect, client)
    f.addErrback(onStateEpConnectErr, args.host, args.port)
    
    d.transport.write(feebackpublish)
